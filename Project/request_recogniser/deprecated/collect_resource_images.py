"""
教学资源图片自动搜集脚本
使用 Selenium 从 Bing Images 搜集 pencil、eraser、block 三种物品图片，每种 300 张。
...
用法：
    python collect_resource_images.py                     # 搜集全部 3 种物品
    python collect_resource_images.py --item pencil       # 只搜集铅笔
    python collect_resource_images.py --count 100         # 每种搜集 100 张
"""
...
# 物品及多组搜索关键词（扩大搜索范围，涵盖不同角度和场景）
ITEM_QUERIES: dict[str, list[str]] = {
    # "pencil": [
    ...
    #     "school pencil",
    #     "yellow pencil",
    #     "writing pencil closeup",
    # ],
    # "eraser": [
    #     "eraser isolated white background",
    #     "pencil eraser rubber",
    #     "white eraser stationery",
    #     "school eraser",
    #     "rubber eraser closeup",
    #     "eraser on white background",
    #     "pink eraser",
    # ],
    "block": [
        "single toy block isolated white background",
        "single wooden block toy",
        "one plastic building block isolated",
        "individual toy block isolated",
        "single children alphabet block",
        "one colorful toy block isolated",
        "single building block white background",
    ],
}
...
MAX_SCROLL_ATTEMPTS = 80  # 单页最大滚动次数
...
class ResourceImageCollector:
    """使用 Selenium 从 Bing Images 搜集教学资源图片（pencil, eraser, block）。"""
    ...
    def collect_item(
        self,
        item_key: str,
        target_count: int = TARGET_COUNT_PER_ITEM,
    ) -> int:
        """
        搜集单个物品的图片，使用多关键词 + 多页搜索扩大图片池。

        Args:
            item_key: 物品键名（如 'pencil'）
            target_count: 目标搜集数量

        Returns:
            实际成功下载的图片数量
        """
        queries = ITEM_QUERIES.get(item_key)
        if not queries:
            print(f"[错误] 未知物品: {item_key}")
            return 0

        save_dir = self.output_root / item_key
        ...
        if already_have >= target_count:
            print(f"[{item_key}] 已有 {already_have} 张，达到目标 {target_count}，跳过")
            return already_have

        needed = target_count - already_have
        print(f"\n{'=' * 60}")
        print(f"[{item_key}] 目标: {target_count} 张")
        print(f"[{item_key}] 已有: {already_have}, 还需: {needed}")
        print(f"[{item_key}] 搜索关键词 ({len(queries)} 组):")
        for q in queries:
            print(f"    - \"{q}\"")
        print(f"{'=' * 60}")
        ...
        for qi, query in enumerate(queries):
            ...
            print(f"\n  [{item_key}] 搜索 {qi + 1}/{len(queries)}: \"{query}\"")
            murls = self._collect_murls_for_query(query)
            total_murls += len(murls)
            print(
                f"  [{item_key}] 收集到 {len(murls)} 个原始图片 URL"
                f"（累计 {total_murls} 个）"
            )
    ...
    def run(
        ...
        Args:
            items: 要搜集的物品列表，None 表示全部
            target_count: 每种的目标数量
        """
        if items is None:
            items = list(ITEM_QUERIES.keys())
        ...
        try:
            for item_key in items:
                self.collect_item(item_key, target_count=target_count)
        finally:
            if self.driver:
                self.driver.quit()
                print("\n浏览器已关闭")
        ...
        for item_key in items:
            ...
            queries = ITEM_QUERIES[item_key]
            print(f"  {item_key} ({queries[0]}): {count} 张")
        print(f"{'=' * 60}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="从 Bing Images 搜集教学资源图片（pencil, eraser, block）",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例：
  python collect_resource_images.py                       # 搜集全部 3 种物品，每种 300 张
  python collect_resource_images.py --item pencil         # 只搜集铅笔
  python collect_resource_images.py --item pencil eraser  # 搜集铅笔和橡皮擦
  python collect_resource_images.py --count 100           # 每种搜集 100 张
  python collect_resource_images.py --no-headless         # 显示浏览器窗口
  python collect_resource_images.py --output ./my_images  # 自定义输出目录
  python collect_resource_images.py --pages 8             # 每个关键词搜索 8 页
        """,
    )
    parser.add_argument(
        "--item",
        nargs="+",
        choices=list(ITEM_QUERIES.keys()),
        default=None,
        help="要搜集的物品类别（默认全部：pencil, eraser, block）",
    )
    parser.add_argument(
        "--count",
        type=int,
        default=TARGET_COUNT_PER_ITEM,
        help=f"每种物品的目标图片数量（默认 {TARGET_COUNT_PER_ITEM}）",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    collector = ResourceImageCollector(
