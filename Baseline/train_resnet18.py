"""
猫品种识别模型 — ResNet-18 迁移学习训练脚本
==============================================

基于 baseline_prompt.md 中的分析：
  - 推荐 CNN 架构（ResNet-18）而非 YOLO，因任务本质是 5 猫品种细粒度分类
...
用法：
    python train_resnet18.py                              # 默认配置训练
    python train_resnet18.py --epochs 80 --batch 32       # 自定义参数
    python train_resnet18.py --data ./my_images            # 自定义数据路径
    python train_resnet18.py --resume checkpoints/xxx.pth  # 从检查点恢复
"""
...
# 修复 SSL 证书验证问题（在某些网络环境下下载预训练权重时出现）
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("training_resnet18.log", encoding="utf-8"),
    ],
)
...
def load_pretrained_state_dict(weights_enum) -> "dict":
    """
    加载 ImageNet 预训练权重，并兼容 SSL 下载失败的环境。
    
    尝试顺序：
      1. torchvision 内置下载器（默认行为）
      2. 若失败（如 SSL 证书 / 网络代理问题），回退为用 requests
         手动下载并跳过 SSL 证书验证（verify=False）
      3. 若 requests 不可用，再回退为 urllib + 未验证的 ssl 上下文
    
    权重下载后缓存于 ~/.cache/torch/hub/checkpoints/，后续运行不会重复下载。
    
    Args:
        weights_enum: torchvision.models 的 Weights 枚举值（如 ResNet18_Weights.IMAGENET1K_V1）
    ...
    # 1) 默认下载器
    try:
        logger.info("  [1/3] 尝试使用 torchvision 内置下载器获取权重...")
        return weights_enum.get_state_dict(progress=True)
    except Exception as e:
        logger.warning(f"  ⚠️ torchvision 内置下载失败: {e}")

    # 解析下载 URL 与本地缓存路径
    import urllib.parse
    url = weights_enum.url
    filename = os.path.basename(urllib.parse.urlparse(url).path)  # e.g. resnet18-f37072fd.pth
    cache_dir = Path(torch.hub.get_dir()) / "checkpoints"
    cache_dir.mkdir(parents=True, exist_ok=True)
    file_path = cache_dir / filename

    if file_path.exists():
        logger.info(f"  命中本地缓存: {file_path}")
        return torch.load(file_path, map_location="cpu")

    # 2) 回退：requests + verify=False
    try:
        logger.info(f"  [2/3] 回退：通过 requests 跳过 SSL 验证下载权重...")
        import requests
        ...
        logger.info(f"  ✅ 已下载权重: {file_path} ({downloaded/1024/1024:.1f} MB)")
        return torch.load(file_path, map_location="cpu")
    except Exception as e:
        logger.warning(f"  ⚠️ requests 下载失败: {e}")

    # 3) 回退：urllib + 未验证 ssl 上下文
    logger.info(f"  [3/3] 最终回退：使用 urllib 下载权重...")
    import urllib.request
    ctx = ssl.create_default_context()
    ctx.check_hostname = False
    ctx.verify_mode = ssl.CERT_NONE
    urllib.request.urlretrieve(url, file_path, context=ctx)
    logger.info(f"  ✅ 已下载权重: {file_path}")
    return torch.load(file_path, map_location="cpu")


def build_model(num_classes: int = 5, freeze_backbone: bool = True) -> nn.Module:
    """
    构建基于 ResNet-18 的迁移学习模型。
    
    步骤：
    1. 加载 ImageNet 预训练的 ResNet-18（自动下载权重，带 SSL 容错）
    ...
    Returns:
        构建好的 PyTorch 模型
    """
    logger.info(f"\n构建模型: ResNet-18 (预训练于 ImageNet)")
    logger.info(f"  输出类别数: {num_classes}")
    
    # 先创建随机初始化的模型结构，再手动加载预训练权重
    # （避免 models.resnet18(weights=...) 直接触发内置下载器导致的 SSL 错误）
    model = models.resnet18(weights=None)
    
    # 加载 ImageNet 预训练权重（自动下载 resnet18-f37072fd.pth，~45MB）
    # 权重文件缓存路径: ~/.cache/torch/hub/checkpoints/
    try:
        state_dict = load_pretrained_state_dict(ResNet18_Weights.IMAGENET1K_V1)
        model.load_state_dict(state_dict)
        logger.info("  ✅ 已载入 ImageNet 预训练权重")
    except Exception as e:
        logger.error(f"  ❌ 预训练权重加载失败: {e}")
        logger.error("    将使用随机初始化权重继续训练（不推荐，精度会下降）")
    
    # 替换分类头 — ResNet-18 的 fc 输入为 512 维（vs ResNet-50 的 2048 维）
    # 原 fc 层: Linear(512, 1000) → 新分类头: 512 → 256 → BatchNorm → 5
    num_features = model.fc.in_features  # 512 (ResNet-18)
...
def main():
    parser = argparse.ArgumentParser(
        description="ResNet-18 猫品种识别模型训练脚本",
    ...
    output_dir = Path(args.output) / f"resnet18_cat_{timestamp}"
    ...
    if vram_gb >= 10:
        ...
        # batch_size 放大（ResNet-18 更轻量，可以用更大的 batch）
        suggested_batch = 96 if vram_gb < 16 else 128
    ...
    total = sum(p.numel() for p in model.parameters())
    logger.info(f"可训练参数: {trainable:,} / {total:,} ({trainable/total*100:.1f}%)")
    fc_trainable = sum(p.numel() for p in model.fc.parameters() if p.requires_grad)
    logger.info(f"  解冻后新增可训练参数: {trainable - fc_trainable:,}")
    ...
    try:
        ...
        script_path = output_dir / "resnet18_cat_scripted.pt"
    ...
    try:
        ...
        onnx_path = output_dir / "resnet18_cat.onnx"
        torch.onnx.export(
    ...
    logger.info(f"  训练日志:     training_resnet18.log")
